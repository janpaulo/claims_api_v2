...
const getHospitals = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Hospital records retrieved successfully!", result });
  } catch (error) {
    console.error("Error retrieving hospital records:", error);
    res.status(500).json({
      error: "Failed to retrieve hospital records. Please try again later.",
    });
  }
};
...
const getHospitalById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Hospital record retrieved successfully!", result });
  } catch (error) {
    console.error("Error retrieving hospital record:", error);
    res.status(500).json({
      error: "Failed to retrieve hospital record. Please try again later.",
    });
  }
};

const getHospitalByIdUsers = async (req, res) => {
  const { id } = req.params;

  try {
    const result = await getRecordById("users", "hci_no", id);

    if (!result || result.length === 0) {
      return res
        .status(404)
        .json({ message: "Hospital users account not found" });
    }

    res.status(200).json({
      message: "Hospital users account record retrieved successfully!",
      users: result,
    });
  } catch (error) {
    console.error("Error retrieving hospital users account record:", error);
    res.status(500).json({
      error:
        "Failed to retrieve hospital users account record. Please try again later.",
    });
  }
};
...
const deleteHospital = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: "Failed to delete hospital account. Please try again later.",
    });
  }
};
